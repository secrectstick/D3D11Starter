...
void Mesh::createBuffers(Vertex* vertices, int* indices) {
	...
	//    be if we want the GPU to act on it (as in: draw it to the screen)
	{
		...
		vbd.ByteWidth = sizeof(Vertex) * this->vertCount;       // 3 = number of vertices in the buffer
...
void Mesh::Draw() {
	...
	// - Other Direct3D calls will also be necessary to do more complex things
	{
		...
		Graphics::Context->DrawIndexed(
			this->idxCount,     // The number of indices to use (we could draw a subset if we wanted)
